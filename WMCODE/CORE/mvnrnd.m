function s = mvnrnd(mu, Sigma, K)
    ...
    ## clean out bogus values from Sigma that make it non positive semi-definite
    _sigma = diag(diag(Sigma));
    Sigma = _sigma + tril(_sigma, -1) + tril(_sigma, -1)';
    [U, p] = chol(Sigma);
    if p ~= 0
       warning("%s: non-positive definite correlation matrix", mfilename);
    endif
    s = randn(n,d)*U + mu;
